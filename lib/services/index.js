...
module.exports.prototype = {
  ...
  _validateUnique: function _validateUnique(req, id, docs, fn) {
    ...
    uniqueFields.forEach(function (key) {
      var check = [];
      docs.forEach(function (doc) {
        if (doc[key]) {
          var val = doc[key];
          val = mongoose.Types.ObjectId.isValid(val.toString()) ? utils.getId(val) : val;

          isUnique = isUnique && docs.every(function (compare) {
            if (compare === doc) {
              return true;
            }
            var c = compare[key];
            c = mongoose.Types.ObjectId.isValid(c.toString()) ? utils.getId(c) : c;
            return c !== val;
          });
