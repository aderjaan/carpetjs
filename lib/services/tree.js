...
module.exports = function TreeService(Model) {
  this.super_.super_ = Base.prototype;
  Base.call(this, Model);
};

utils.inherits(module.exports, Base);
...
_.extend(module.exports.prototype, {
  ...
  buildTree: function buildTree(root, records) {
    var self = this;
    var children = _.filter(records, function (item) {
  ...
  copy: function copy(req, doc, newParentId, fn) {
    ...
    async.waterfall([function (callback) {
      ...
      docs.forEach(function (doc) {
        ...
        delete clone._id;
        modelMap[doc._id] = new self._model(clone);
      });

      _.values(modelMap).forEach(function (model) {
  ...
  remove: function remove(req, id, fn) {
    ...
    self.getSelfAndDescendants(req, id, function (err, docs) {
      ...
      var item = _.find(docs, { _id: id });
      if (!item) {
        return fn(new Error('invalid_id_provided'));
      }

      var isRoot = !!!item.parentId;

      if (item.use_counter) {
        return fn(new Error('item_in_use'));
      }
      else if (docs.length > 1) {
        if (_.any(docs, 'use_counter')) {
          return fn(new Error('item_in_use'));
        }
      }
      var itemsToDelete = _.pluck(docs, '_id');
  ...
  getSelfAndDescendants: function getSelfAndDescendants(req, id, fn) {
    var self = this;
    var cacheKey = req.organizationId + id;
    ...
    utils.cache.get(cacheKey, function (err, data) {
      ...
      self.findOne(req, {
        ...
        if (!doc) {
          var modelName = _.last(self._model.modelName.split('.'));
  ...
  getTree: function getTree(req, id, fn, fields) {
    ...
    this.findOne(req, {
      ...
      self.find(req, {
        ...
        var top = _.find(docs, { _id: root._id });
        if (!top) {
          return fn(new Error('root_node_not_found'));
        }
        return fn(err, self.buildTree(top, docs));
      });
    });
  },
