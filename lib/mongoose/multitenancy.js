...
var attachOrganizationId = function attachOrganizationId(req, conditions) {
  var model = this;
  var schema = model.schema || (model.model && model.model.schema);
...
module.exports = function (mongoose) {
  ...
  mongoose.Model.prototype.save = function (req) {
    ...
    if (validRequest(req) && req.userId) {
  ...
  mongoose.Model.aggregate = function(req) {
    ...
    if (args.length) {
      ...
      if (first_op.hasOwnProperty("length")) { //Is an array of Operations
        if (!first_op.length || !first_op[0].$match) {
          first_op.unshift({ $match: {} });
        }

        attachOrganizationId.call(this, req, first_op[0].$match);
        args[0] = first_op;
      }
      else if (!first_op.$match) {
        args.unshift({ $match: {} });
