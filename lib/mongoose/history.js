...
var saveHistory = function saveHistory(schema, model, req) {
  ...
  modified.forEach(function (key) {
    ...
    if (utils.isEqual(to, from) || utils.idEquals(to, from)) {
      return;
    }
  ...
  parse: function (req, data, appName, model, cb) {
    var contains = function (list, o) {
      if (utils.isObjectId(o._id || o)) {
        ...
        return list.filter(function (oo) {
          return utils.isEqual(oo, o);
        }).length > 0;
      }
    };

    var changes = data.reduce(function (changes, d) {
      return changes.concat(d.changes);
    }, []);

    // map is a hash that contains all unique object id's
    // for every 'type'
    var map = changes.reduce(function (map, change) {
      ['from', 'to'].forEach(function (name) {
        var arr = utils.getId(Array.isArray(change[name]) ? change[name] : [change[name]]);
        if (utils.isObjectId(arr)) {
