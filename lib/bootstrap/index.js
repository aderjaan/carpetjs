...
module.exports = {
  ...
  */
  isApiCall: function () {
    ...
    for (var i = 0; i < call.length; i++) {
      var dir = path.dirname(call[i].getFileName());
      if (/(carpetjs[\/\\]lib|[\/\\]apps[\/\\]([^\/\\]*))[\/\\]api/.test(dir)) {
        return true;
      }
    }
    return false;
  ...
  bootstrap: function (dir) {
    ...
    fs.readdirSync(dir).forEach(function (file) {
      ...
      if (stats && stats.isDirectory()) {
        ...
        if (RegExp.$1 === 'index') {
          CACHE.BOOTSTRAP[dir] = utils.extend(CACHE.BOOTSTRAP[dir], self.require(lookup));
        }
        else {
   ...
   */
  baseService: function (model, prototype, type) {
    type = type || 'index';
    if (typeof model === 'string') {
      model = module.exports.model(model);
    }

    var Base = require('../services/'.concat(type));
    var Service = function (Model) {
      this.super_ = Base.prototype;
      Base.call(this, Model);
    };
    utils.inherits(Service, Base);
    utils.extend(Service.prototype, prototype);
   ...
   */
  baseApi: function (serviceName, serviceApp, prototype, type) {
    type = type || 'index';
    ...
    var Base = require('../api/'.concat(type));
    var Api = function (serviceName, serviceApp) {
      this.super_ = Base.prototype;
      Base.call(this, serviceName, serviceApp);
    };
    utils.inherits(Api, Base);
    utils.extend(Api.prototype, prototype);
