...
module.exports.prototype = {
  ...
  _commit (model, req, method, ...args) {
    ...
    $.utils.cache.wrap(options.key, fn => model[method](...args, fn), options, (err, data) => {
      if (!data) {
        return fn(err, data);
      }
      else if (!lean) {
  ...
  _validateUnique (req, id, docs, fn) {
    ...
    const error = keys => ({
      ...
      message: `An item already exists with the same value for "${(keys || []).join('/')}"`
    });
    ...
    return this.count(req, conditions, (err, count) =>
      fn(err || count > 0 ? error(uniqueFieldsContext) : null));
  },
    ...
    if (params.filter) {
      ...
      params.conditions.$or = searchFields.map(key => {
        ...
        if (key === '$text') {
          ...
          const number = parseFloat(filter);
  ...
  _cachePrefix (req, options) {
    let prefix = '';

    if (req && options && options.reqFields) {
      options.reqFields.forEach(key => {
        if (req[key]) {
          prefix = `${req[key]}${prefix ? '_' : ''}${prefix}`;
        }
      });
    }

    return `${this._model.modelName.toLowerCase()}_${prefix}`;
  },
  ...
  _inflate (data, Model) {
    ...
    if (Array.isArray(data)) {
      return data.map(data => this._inflate(data, Model));
    }
    ...
    Object.keys(data).forEach(key => {
      ...
      if (!from.every(isString) && to.every(isString)) {
        ...
        const ref = path && (
          ...
          (path.caster && path.caster.options && path.caster.options.ref));
        const subModel = ref && $.mongoose.model(ref);

        if (subModel) {
          data[key] = this._inflate(data[key], subModel);
        }
