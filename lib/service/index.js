...
var evaluateSort = function (sort) {
...
var evaluateFields = function (fields, service, single) {
...
module.exports.prototype = {
  ...
  create: function create(req, doc, fn) {
    ...
    this._createModel(req, doc, function (err, model) {
      ...
      self._commit(model, req, 'save', function (err) {
  ...
  batchCreate: function batchCreate(req, docs, fn) {
    ...
    this._validateCreate(req, docs, function (err) {
      ...
      self._commit(self._model.collection, req, 'insert', docs, function (err) {
        if (err) {
          return self.remove({
  ...
  update: function update(req, conditions, changes, fn) {
    ...
    this._validateUpdate(req, id, changes, function (err) {
      ...
      if (isModel) {
        ...
        return self._commit(conditions, req, 'save', fn_);
      }
      else {
        if (single) {
        ...
        self._commit(req, 'update', conditions, {
  ...
  remove: function remove(req, conditions, fn) {
    ...
    var confirmed = req.query && req.query.confirmed === '1';
    var dependencies = this.removeDependencies || [];

    var prevents = dependencies.filter(function (d) {
      return d.execute === 'prevent';
    });
    ...
    var execute = function () {
      ...
      this.find(req, {
        ...
        var done = 0;
        var removed = 0;
        var fields = parseFields(self.uniqueFields);
        ...
        items.forEach(function (item) {
          ...
          fields.forEach(function (field) {
            if (self._model.schema.paths[field].options.type !== String) {
              return;
            }
          ...
          var cond = { _id: $.utils.getId(item) };

          if (dry) {
            return self._commit(req, 'findOne', cond, fn_);
          }

          return self._commit(req, 'update', cond, update, fn_);
        });
      });
    }.bind(this);
    ...
    var checkDependencies = function () {
      ...
      return dependencies.forEach(function (dependency) {
        ...
        if (!confirmed) {
          return dependency.query(req, conditions, function (err, data) {
            data = err ? [err] : data;
        ...
        dependency.execute(req, conditions, function (err) {
          error = error || err;
    ...
    var errors = {};
    var done = 0;
    return prevents.forEach(function (prevent) {
      prevent.query(req, conditions, function (err, data) {
        data = err ? [err] : data;
  ...
  _commit: function _commit(model, req, method) {
    var self = this;
    ...
    if (['find', 'findOne', 'aggregate', 'count', 'distinct'].indexOf(method) < 0) {
      var cacheKey = this._cachePrefix(req, this.cache);
      var fn = originalArgs[originalArgs.length - 1];

      if (typeof fn === 'function') {
        originalArgs[originalArgs.length - 1] = function () {
          var args = arguments;
          $.utils.cache.delMatch(cacheKey, function () {
            fn.apply(null, args);
          });
        };
      }

      return model[method].apply(model, originalArgs);
    }

    var fn = args[args.length - 1];
    args = args.slice(0, args.length - 1);

    var req = args[0] && args[0].hasOwnProperty('originalMethod') && req;
    var cleanArgs = (req ? args.slice(1) : args);
    var params = cleanArgs[2];
    var options = params && params.cache;
    var lean = !params || !!params.lean;
    ...
    if (!options.key) {
      options.key = this._cachePrefix(req, options)
        .concat('_', $.utils.hash([].concat(cleanArgs)));
    }

    return $.utils.cache.wrap(options.key, function (fn) {
      return model[method].apply(model, args.concat(fn));
    }, options, function (err, data) {
      if (!data) {
        return fn(err, data);
      }
      else if (!lean) {
        data = this._inflate(data);
      }
  ...
  _strip: function _strip(doc, whitelist) {
    ...
    return parseFields(whitelist).reduce(function (d, key) {
  ...
  _validateUnique: function _validateUnique(req, id, docs, fn) {
    ...
    var lowerCase = !!this.uniqueFieldsLowerCase;
    var uniqueFields = parseFields(this.uniqueFields);
    var conditions = [];
    var error = function () {
      return {
    ...
    conditions.forEach(function (condition) {
      Object.keys(condition).forEach(function (key) {
        if (key !== '$or' && Array.isArray(condition[key])) {
          condition.$or = condition.$or || [].concat(condition[key].map(function (sub) {
            var match = {};
            match[key] = {
              $elemMatch: sub
            };
    ...
    var uniqueFieldsContext = parseFields(this.uniqueFieldsContext);
  ...
  _find: function _find(req, params, fn, single) {
    ...
    var defaultSort = evaluateSort(this.defaultSort);
    ...
    if (params.filter) {
      var searchFields = parseFields(params.searchFields || this.defaultSearchFields);
      ...
      params.conditions.$or = searchFields.map(function (key) {
        ...
        if (key === '$text') {
          condition[key] = { $search: filter };
        }
        else if (path && (path.instance === 'Number')) {
          var number = parseFloat(filter);
          if (isNaN(number)) {
            return undefined;
          }
          ...
          condition[key] = $.utils.filterToRegExp(filter);
        }
    ...
    var filterFields = parseFields(this.filterFields).reduce(function (fields, field) {
      if (/^(.*?\.)\*$/.test(field)) {
        var prefix = RegExp.$1;
    ...
    filterFields.forEach(function (key) {
      ...
      if (path && (path.instance === 'Number' || path.instance === 'Date')) {
        filter = filter.map(function (value) {
          ...
          if (path.instance === 'Number') {
            ...
            var offset = req.headers['x-utc-offset'] || '+00:00';
            var dummy = '2016-01-01T00:00:00.000';
            var offset = (Date.parse(dummy.concat(offset)) - Date.parse(dummy)) / 60000;
      ...
      var complex = filter.length > 1 && filter.some(function (value) {
    ...
    this._commit(req, method, params.conditions, params.fields, params, function (err, data) {
      if (err || !data) {
        return fn && fn(err);
      }
  ...
  _cachePrefix: function (req, options) {
    var prefix = '';

    if (req && options && options.reqFields) {
      options.reqFields.forEach(function (key) {
        if (req[key]) {
          prefix = ''.concat(req[key], prefix ? '_' : '', prefix);
        }
      });
    }

    return this._model.modelName.toLowerCase().concat('_', prefix);
  },

  // Inflates an object to a mongoose model
  _inflate: function (data, Model) {
    Model = Model || this._model;

    if (Array.isArray(data)) {
      return data.map(function (data) {
        return this._inflate(data, Model);
      }.bind(this));
    }
    ...
    Object.keys(data).forEach(function (key) {
      var string = function (item) {
        return typeof item === 'string' || $.utils.isObjectId(item.toString());
      };

      var from = [].concat(data[key] || []);
      var to = [].concat(model[key] || []);

      if (!from.every(string) && to.every(string)) {
        model.setValue(key, data[key]);
      }
    });

    model._doc = Object.keys(model._doc).reduce(function (doc, key) {
