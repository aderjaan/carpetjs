  ...
  remove: function remove(req, conditions, fn) {
    ...
    var execute = function () {
      if (!this._model.schema.paths.inactive) {
        if (dry) {
          return this.commit(req, 'find', conditions, fn);
        }
      ...
      this.find(req, {
        ...
        items.forEach(function (item) {
          ...
          if (dry) {
            return self.commit(req, 'findOne', cond, fn_);
          }

          return self.commit(req, 'update', cond, update, fn_);
        });
      });
    }.bind(this);
      ...
      if (path && (path.instance === 'Number' || path.instance === 'Date')) {
        filter = filter.map(function (value) {
          ...
          if (path.instance === 'Number') {
            ...
            offset = (Date.parse(dummy.concat(offset)) - Date.parse(dummy)) / 60000;
