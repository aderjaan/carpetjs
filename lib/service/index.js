...
module.exports.prototype = {
  ...
  _commit (model, req, method, ...args) {
    ...
    $.utils.cache.wrap(options.key, fn => model[method](...args, fn), options, (err, data) => {
      if (!data) {
        ...
        data = this._hydrate(data, this._model, params);
      }
  ...
  _hydrate (data, Model, params) {
    if (Array.isArray(data)) {
      return data.map(data => this._hydrate(data, Model, params));
    }
    ...
    Object.keys(data).forEach(key => {
      ...
      if (!from.every(isString) && to.every(isString)) {
        ...
        if (subModel) {
          data[key] = this._hydrate(data[key], subModel, params);
        }
