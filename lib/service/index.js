...
var inflateModel = function (constructor) {
  return function (data) {
    var model = new constructor(data);
    model.$__reset();
    model.isNew = false;
    return model;
  };
...
module.exports.prototype = {
  ...
  _commit: function _commit(model, req, method) {
    ...
    if (options === false || options === 0 || options && options.ttl === 0) {
      return model[method].apply(model, originalArgs);
    }
    ...
    if (!options.key) {
      options.key = $.utils.hash([].concat(cleanArgs));

      if (req && options.reqFields) {
        options.reqFields.forEach(function (key) {
          if (req[key]) {
            options.key = req[key].concat('_', options.key);
          }
        });
      }
    }
    ...
    }, options, function (err, data, fromCache) {
      if (!data) {
        return fn(err, data);
      }

      var list = [].concat(data).map(function (item) {
        if (lean || !!item.save) {
          return item;
        }

        return inflateModel(self._model)(item);
      });

      return fn(null, Array.isArray(data) ? list : list[0]);
    });
  },
