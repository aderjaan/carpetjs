...
module.exports.prototype = {
  ...
  create: function create(req, doc, fn) {
    ...
    this._validateCreate(req, doc, function (err) {
      ...
      self.commit(model, req, 'save', function (err, item) {
        if (err) {
          return fn(err);
        }
  ...
  update: function update(req, conditions, changes, fn) {
    ...
    if (!!conditions.save) {
      return this._validateUpdate(req, conditions.id, changes, function (err) {
        ...
        return self.commit(conditions, req, 'save', function (err, data) {
          if (err) {
            return fn(err);
          }
    ...
    if ($.utils.isObjectId($.utils.getId(conditions))) {
      conditions = { _id: $.utils.getId(conditions) };
    }
    this.commit(req, 'update', conditions, {
      ...
      self._parseOutput(data, req, function (data) {
        fn(null, data);
      });
    });
  },
  ...
  _find: function _find(req, params, fn, single) {
    ...
    this.commit(req, method, params.conditions, params.fields, params, function (err, data) {
      if (err || !data || params.parse === false) {
