...
module.exports.prototype = {
  ...
  _commit (model, req, method, ...args) {
    ...
    if (!['find', 'findOne', 'aggregate', 'count', 'distinct'].includes(method)) {
      ...
      if (typeof fn === 'function') {
        originalArgs[originalArgs.length - 1] = (...args) => {
          const prefixes = this._cachePrefix(req, this.cache, 2);
          $.logging.verbose(req, `Invalidating cache: ${prefixes}`);
          $.utils.cache.delMatch(prefixes, () => fn(...args));
        };
      }
    ...
    $.logging.verbose(req, `Query: ${this._model.modelName}`);
    $.utils.cache.wrap(options.key, fn => {
      $.logging.verbose(req, `Uncached: ${options.key}`);
      return model[method](...args, fn);
    }, options, (err, data) => {
      if (!data) {
        return fn(err, data);
      }
      ...
      $.logging.verbose(req, `Cached: ${options.key}`);
      return fn(null, data);
    });
  },
