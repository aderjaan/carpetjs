...
module.exports.prototype = {
  ...
  remove: function remove(req, conditions, fn) {
    ...
    var prevents = this.preventRemoves || [];

    var remove_ = function () {
      delete conditions._validated;
    ...
    if (prevents.length && !conditions._validated) {
      var errors = {};
      var done = 0;
      return prevents.forEach(function (prevent) {
        prevent.query(req, conditions, function (err, count) {
          count = err ? 1 : count;

          if (count) {
            errors[prevent.name] = err ? 1 : count;
          }
