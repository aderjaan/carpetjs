...
module.exports.prototype = {
  ...
  _commit: function _commit(model, req, method) {
    ...
    var cacheKey = [].concat(args.slice(1));
    var cacheOptions = (req ? args[1].cache : args[0].cache) || this.cache || { ttl: 60 };

    if (req && cacheOptions.reqFields) {
      req = cacheOptions.reqFields.reduce(function (cached, key) {
        if (req[key])
        cached[key] = req[key];
        return cached;
      }, {});
    ...
    return $.utils.cache.wrap(cacheKey, function (fn) {
