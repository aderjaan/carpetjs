...
module.exports.prototype = {
  ...
  remove: function remove(req, conditions, fn) {
    ...
    this.find(req, {
      ...
      var done = 0;
      var removed = 0;
      var fields = self.uniqueFields.split(' ');
  ...
  _validateUnique: function _validateUnique(req, id, docs, fn) {
    ...
    return this.count(req, conditions, function (err, count) {
      return fn(err || count > 0 ? 'duplicate_unique_fields' : null);
    });
  ...
  _find: function _find(req, params, fn, single) {
    ...
    params.sort = $.utils.extend({}, params.defaultSort || {}, params.sort || {});

    if (params.sort_by) {
      var sort_types = params.sort_type && params.sort_type.split(',') || [];
      params.sort_by.split(',').forEach(function (key, i) {
        if (/^-(.*)$/.test(key)) {
          params.sort[RegExp.$1] = -1;
        }
        else {
          params.sort[key] = parseInt(sort_types[i] || '1', 10) || 1;
        }
      });
