...
module.exports = {
  ...
  */
  camelize: function (name, capitalize) {
    ...
    return name.replace(/-([a-z])/g, function (a, key) { return key.toUpperCase(); });
  },
  ...
  */
  ...
  /*
    ...
    Can be extended using extensions
   */
  baseService: function (model, extensions, type) {
    type = type || 'index';
    var base = this.service('base/'.concat(type), 'shared');
    if (typeof model === 'string') {
      model = this.model(model);
    }
    return base(model, extensions);
  },
  ...
  /*
    ...
    Can be extended using extensions
   */
  baseApi: function (serviceName, serviceApp, extensions) {
    if (typeof serviceApp !== 'string') {
      extensions = serviceApp;
      serviceApp = this.appName();
    }
    return this.api('base', 'shared').call(null, serviceName, serviceApp, extensions);
  }
