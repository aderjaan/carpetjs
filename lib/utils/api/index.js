...
var parseError = function (err) {
  ...
  var def = config.errors[err.message] || config.errors.unknown;
...
module.exports.error = function (res, err, code) {
  ...
  err = parseError(err);
  code = parseInt(err.httpcode || 500, 10);
  ...
  var args = json ? [err] : ['error', err];

  return res.status(code)[handle].apply(res, args);
};
 ...
 */
module.exports.wrap = function (res, fn, errorCode) {
  return function (err, data) {
    if (err) {
      return module.exports.error(res, err, errorCode);
    }
...
/**
 * Maps data to json, including virtuals if available
 *
 * @param {object} data
 */
module.exports.toJSON = function (data) {
  ...
  return data.toObject ? data.toObject({ virtuals: true }) : data;
};
