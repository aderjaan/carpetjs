...
exports.stringify = o => JSON.stringify(o, replacer, 2);

/*
 * Error tracing
 */
exports.traceError = (req, err, expose) => {
  if (expose && expose !== 'trace' && $.sentry) {
    const captured = err instanceof Error ? err : $.utils.stringify(err);

    $.sentry.captureException(captured, { req }, (captureError, id) => {
      if (captureError) {
        $.logging.error('Failed to capture register error in Sentry');
        return exports.traceError(req, err, 'trace');
      }

      return $.logging.error(`Captured exception to Sentry: ${id}`);
    });
  }

  const shouldTrace = expose === 'trace' || $.config.env === 'development' ||
...
const validateEmailRx = new RegExp([
  '^[a-zäöüÄÖÜß0-9!#$%&\'*+/=?^_`{|}~-]+',
  '(?:\\.[a-zäöüÄÖÜß0-9!#$%&\'*+/=?^_`{|}~-]+)*',
  '@(?:[a-zäöüÄÖÜß0-9](?:[a-zäöüÄÖÜß0-9-]*[a-zäöüÄÖÜß0-9])?\\.)+',
  '[a-zäöüÄÖÜß0-9](?:[a-zäöüÄÖÜß0-9-]*[a-zäöüÄÖÜß0-9])?$'].join(''));
