...
try {
  settings = utils.extend({}, settings, require(root.concat('config/default.json')));
} catch (ex) {}
...
try {
  ...
  settings = utils.extend({}, settings, require(envPath));
} catch (ex) {}
...
// Load bootstrapped utils
global.utils = $.bootstrap(__dirname.concat('/../utils'));

// Extend local utils
try {
  utils.extend(global.utils, $.bootstrap(config.root.concat('utils')));
} catch (ex) {}
